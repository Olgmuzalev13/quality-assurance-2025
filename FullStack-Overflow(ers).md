...
* [Правила валидации](#правила-валидации)
  ...
  * [Контакты](#контакты)
  * [Аватар](#аватар)
* [Правила валидации JIT форм](#правила-валидации-jit-форм)
...
Ограничение длины ввода: 50 символов.
...
Валидным именем или фамилией считается слово (набор букв) или слова, разделенные
символом дефиса (-) на русском или английском языке.
...
Ограничение длины ввода: 500 символов.

### Сайт компании

Валидным сайтом считается любая строка, обладающая префиксом https:// или http://.
...
## Правила валидации JIT форм

Формы, для которых явно указано, обладают Just-In-Time
валидацией. Формат валидации удовлетворяет критериям:
* Валидация активируется при каждом снятии фокуса с инпута
* Инпут валидирует тот ввод, который был в нем на момент
  активации валидации
* Если ввод в инпуте является обязательным, то для
  пустого ввода инпут выводит сообщение об ошибке:
  "Заполните это поле". Для необязательных вводов такого
  не происходит.
...
5. Все поля формы обладают Just In Time (JIT) валидацией согласно
   [правилам валидации](#правила-валидации-jit-форм)
...
11. Поле имени позволяет ввести составное имя

![image](./img/registration/double_name.jpg)

12. Поле имени не позволяет вводить слова, разделенные не дефисами

> $\color{red}Баг$
> Поле имени позволяет вводить слова, разделенные пробелами

![image](./img/registration/two_words_in_name.jpg)

13. Поле имени не позволяет вводить запрещенные
...
14. Поле фамилии позволяет ввести слово на русском языке

![image](./img/registration/russian_surname.jpg)

15. Поле фамилии позволяет ввести слово на английском языке

![image](./img/registration/english_surname.jpg)

16. Поле фамилии позволяет ввести составную фамилию

![image](./img/registration/double_surname.jpg)

17. Поле фамилии не позволяет вводить слова, разделенные не дефисами

> $\color{red}Баг$
> Поле фамилии позволяет вводить слова, разделенные пробелами

![image](./img/registration/two_words_in_surname.jpg)

18. Поле фамилии не позволяет вводить запрещенные
   [правилами валидации](#правила-валидации) символы.

> $\color{red}Баг$
> Поле фамилии позволяет вводить любые символы

![image](./img/registration/invalid_symbols_in_surname.jpg)

19. Электронная почта "aboba" (или любая другая, не содержащая @) не является корректной

![image](./img/registration/email_aboba.jpg)

20. Электронная почта "a@", имеющая буквы перед @, является корректной

![image](./img/registration/email_a@.jpg)

21. Электронная почта "@" (или любая другая, не содержащая букв перед @) не является корректной

![image](./img/registration/email_@.jpg)

22. Электронная почта "a@mail.ru", имеющая буквы слева и справа от @, является корректной

![image](./img/registration/email_asobaka.jpg)

22. Пароль менее 8 символов не является корректным для
   input "Пароль" и помечается сообщением
   "Введите пароль длиной хотя бы 8 символов"

![image](./img/registration/invalid_password.jpg)

23. Пароль '1234 678', удовлетворяющий [требованиям валидации](#правила-валидации),
   является корректным

![image](./img/registration/valid_password.jpg)

24. Валидация поля "Повторите пароль" указывает сообщение
   об ошибке, если ввод не совпадает с тем, что указано в
   поле "пароль".
...
25. Нажатие на кнопку "Зарегистрироваться" при наличии
   некорректного ввода ничего не делает.

26. Нажатие на кнопку "Зарегистрироваться" при успешной
   регистрации логинит нового пользователя.

27. Нажатие на кнопку "Зарегистрироваться" при успешной
   регистрации переводит пользователя на главную
   страницу (https://uart.site).

![image](./img/registration/registration_button.jpg)

28. Нажатие на кнопку "Зарегистрироваться" при ошибке
...
2. JIT-валидация ведет себя с обязательными полями так,
   как указано в [правилах валидации](#правила-валидации-jit-форм)
...
3. Поле "Дата рождения" для ввода даты из будущего
   (текущая дата + год), указывает сообщение: "Введена некорректная дата"

> $\color{red}Баг$
> Для ввода даты из будущего указывается сообщение "Вы слишком молоды, чтобы
>  пользоваться сайтом".

![image](./img/registration/date_from_future.jpg)

4. Поле "Дата рождения" для ввода даты рождения
   несовершеннолетнего (< 18 лет) указывает сообщение
   "Вы слишком молоды, чтобы пользоваться сайтом".

![image](./img/registration/date_too_young.jpg)
...
3. Должность, удовлетворяющая [правилам валидации](#правила-валидации),
   не вызывает ошибок (Ведущий инженер-программист)

![image](./img/registration/correct_position.jpg)

4. Должность, содержащая хотя бы один из запрещенных символов, помечается
...
5. Название компании, удовлетворяющее [правилам валидации](#правила-валидации),
...
6. Некорректное название компании помечается ошибкой "Введите название компании на
...
7. Input "Описание компании" позволяет ввести любой текст.
...
8. Input "Сайт компании" позволяет ввести строку, удовлетворяющую [правилам валидации](#правила-валидации)

![image](./img/registration/valid_website.jpg)

9. Input "Сайт компании" не позволяет вводить строки без префиксов https:// или http://

> $\color{red}Баг$
> Некорректный сайт может быть введен  
...
9. Email "@" (или любой другой, не содержащий символов перед и после @) не является валидным.

10. Email "a@" (или любой другой, имеющий символов перед @) является валидным.

11. Email "@a" (или любой другой, не содержащий символов перед @) не является валидным.

12. Email "' '@a" (или любой другой, удовлетворяющий [требованиям валидации](#правила-валидации))
    является валидным.
...
14. При вводе данных ранее зарегистрированного пользователя
...
5. При нажатии на флажок у вакансии, которая уже находится в избранном (флажок уже горит желтым) происходит удаление вакансии из избранного (безвозвратно удаляет вакансию из избранного)  

![alt text](./img/vacancy_page/favoriting_flag.png)

7. При вводе в поле "Заработная плата" отрицательного числа появляется предупреждающая надпись "Сюда нужно ввести целое положительное число" и завершение создания вакансии становится не доступным (кнопка создать вакансию не создает такую вакансию).  
![alt text](img/vacancy_page/salary.png)

8. Отображается drop-down меню с видом работы "Постоянная", "Временная", "Разовая".  
![alt text](img/vacancy_page/vidi_rabot.png)

9. Отображается drop-down меню с видом работы "Художник", "Дизайнер", "Музыкант", "Фотограф", "Видеограф", "Артист/Актёр", "Писатель".  
...
11. Поле "Должность" должно удовлетворять [правилам](#правила-валидации) валидации
...
13. Отображается кнопка "Сохранить" - сохраняющая изменения.
14. Отображается кнопка "В профиль" - отменяющая изменения и перенаправляющая [в профиль](https://uart.site/me?from=vacancyList).
...
9. Поле "Должность" удовлетворяет [правилам](#правила-валидации) валидации:

![img](img/cv_page/cv_position_ru.png)

10. В случае ввода в поле "Должность" данных, не соответствующих [правилам валидации](#правила-валидации), появляется сообщение об ошибке, что данные некорректны.
...
11. При вводе некорректной строки в поле "Должность" выводится сообщение об ошибке.
...
12. Поле "Перевод должности на английский" удовлетворяет [правилам](#правила-валидации) валидации: валидным переводом считается любая комбинация символов на английском языке с пробелами и знаками "-".

![img](img/cv_page/cv_position_english_validation.png)
![img](img/cv_page/cv_position_english.png)

13. Поле "Описание и достижения" удовлетворяет [правилам](#правила-валидации) валидации: валиден любой текст.

![img](img/cv_page/cv_description_ok.png)
![img](img/cv_page/cv_description.png)

14. Поле "Опыт работы" удовлетворяет [правилам](#правила-валидации) валидации: валиден любой текст.
